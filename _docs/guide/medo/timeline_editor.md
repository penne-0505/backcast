...
21. 「現在のタイムラインを保存」ボタンで、現在の目標時刻と行動一覧をテンプレートとして保存する
...
- 遠い位置へ block を移動するときは、移動ハンドルを押さえたまま一時俯瞰に入ってから上下へ動かすと、移動先の前後関係を確認しやすい
...
- 並び替えの開始タイミングがわかりづらい:
  一時俯瞰への縮小は、移動ハンドルに触れた瞬間ではなく、短い hold が成立した時点で始まる。実際の並び替え判定は縮小後 layout が先に反映されるよう、縮小開始と高さアニメーション完了より後に成立する
- ドラッグ中に細かく合わせにくい:
