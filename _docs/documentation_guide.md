...
6. **`_docs/standards/ui_layering.md`**
   - Medo UI の layer model と入力イベント所有権を定義しています。

## 現在の主要プロジェクトドキュメント

- `README.md`: Medo の現在機能、setup、runtime 概要。
- `_docs/guide/medo/timeline_editor.md`: 現在の UI 操作と運用上のコツ。
- `_docs/guide/medo/privacy_policy_operations.md`: data / billing / account deletion 変更時の privacy policy 更新条件。
- `_docs/reference/medo/timeline_domain_reference.md`: timeline domain と state API の仕様。
- `_docs/intent/medo/reverse_timeline_interaction_model.md`: 逆算 UI と state design の判断理由。
- `_docs/guide/flutter/environment_setup.md`: Flutter 実行環境の setup 補足。
