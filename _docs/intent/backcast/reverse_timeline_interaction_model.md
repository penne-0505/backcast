...
`Backcast` は「いまから何を積むか」ではなく、「この時刻に間に合わせるには何が必要か」を先に考えるユースケースを扱います。
...
- アプリ状態は `TimelineNotifier` に集約し、永続化なしのインメモリ構成とする
...
- 永続化が必要になった段階で、`TimelineState` のシリアライズ方式と保存先を設計する
